...
#---------------------------------------------------------------
class ahkfindrequiredversion(sublime_plugin.TextCommand):
    def run(self, edit):
        view = self.view
        requires_version = ""

        for region in view.find_all("(?i)^#Requires AutoHotkey v(\d+\.\d+)"):
            match = re.search(r"v((\d+)(?:\.\d+)+)", view.substr(region))
            
            if match:
                requires_version = match.group(2)
                break
                
        if requires_version:
            # print("Requires Version:", requires_version)
            return requires_version
        else:
            # print("No Requires Version Found")
            return "2"
...
class ahkrun(sublime_plugin.WindowCommand):
	def run(self, version="default"):
		...
		# Call the ahkfindrequiredversion function and store its return value
		required_version_finder = ahkfindrequiredversion(self.window.active_view())
		required_version = required_version_finder.run(self.window.active_view())

		if required_version == "1":
			AHKMajorVersion = "AutoHotKeyExePath"
		elif required_version == "2":
			AHKMajorVersion = "AutoHotKeyV2ExePath"
		else:
			AHKMajorVersion = "AutoHotKeyV2ExePath"

		AutoHotKeyExePath = ""

		AutoHotKeyExePath = sublime.load_settings("AutoHotkey.sublime-settings").get(AHKMajorVersion)[version]
...
class ahkcompile(sublime_plugin.WindowCommand):
	def run(self, version="default"):
		...
		AutoHotKeyBasePath = sublime.load_settings("AutoHotkey.sublime-settings").get(AHKMajorVersion)[version]
		...
		if not os.path.isfile(Ahk2ExePath):
			...
			filepath = self.window.active_view().file_name()
			cmd = [Ahk2ExePath, "/gui", "/in", filepath, "/base", AutoHotKeyBasePath]
