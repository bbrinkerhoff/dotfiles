# Source: https://docs.folivora.ai/docs/keyboard-shortcuts/key-sequence-snippet-import

- [
- [Keyboard Shortcuts
- Key Sequence Snippet Import (CSV / JSON)
...
![alt text](/assets/images/key-sequence-snippet-import2026-03-18T22:12:15.017Z-99f2db6f443c6378d6c8417d0cda8d9e.png)
...
- Converts each character of the trigger text into the correct macOS key code (using the current keyboard layout).

- Builds the full key sequence structure (key-down events, key-up events, and interleaved mixed events).

- Handles **uppercase letters** automatically by grouping consecutive uppercase characters under a single Shift key press.

- Sets `deleteAfterwards` equal to the trigger length, so the typed trigger text is removed before the replacement is pasted.

- Creates a global Key Sequence trigger with the appropriate paste or JavaScript action.
...
- Evaluates each variable in order (date formatting, shell commands, clipboard, etc.)

- Substitutes `{{varname}}` placeholders in the replacement template

- Applies case propagation if enabled

- Handles `$|$` cursor positioning

- Calls `paste_text()` to insert the result
...
`
```
[PreviousKey Sequence Triggers[NextTouch Bar Triggers
