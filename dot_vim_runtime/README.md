...
or add the command to `~/.vim_runtime/vimrcs/custom_config.vim` (see [below](#how-to-include-your-own-stuff)),
...
create an empty `~/.vim_runtime/vimrcs/custom_config.vim` file for further customization.
This file's syntax matches `vimrc` syntax,
and add `vimrc` lines like `set number` as needed.

For instance, my `custom_config.vim` looks like this:

	~/.vim_runtime/vimrcs/custom_config.vim
